...
if [ -d "${AGH_ROOT}/aghues" ]; then
  ...
  if sudo -n true 2>/dev/null || [ -t 0 ]; then
    # Terminal or cached sudo session
    sudo CFLAGS="-O2 -Wno-error=int-conversion -Wno-error=implicit-function-declaration -fpermissive" ./install.sh "$@"
  elif command -v pkexec >/dev/null 2>&1; then
    # Graphical prompt fallback
    pkexec sh -c 'CFLAGS="-O2 -Wno-error=int-conversion -Wno-error=implicit-function-declaration -fpermissive" ./install.sh "$@"' _ "$@"
  else
    echo -e "${RED}ERROR: Neither sudo nor pkexec available.${RES}"
    exit 1
  fi
